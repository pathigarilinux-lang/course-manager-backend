...
app.post('/courses/:id/import', async (req, res) => {
  ...
  try {
    for (const s of students) {
      ...
      if (check.rows.length > 0) { skipped++; } 
      else {
...
// EDIT PARTICIPANT (NEW!)
app.put('/participants/:id', async (req, res) => {
  const { id } = req.params;
  const { full_name, phone_number, status, room_no, dining_seat_no, pagoda_cell_no } = req.body;
  try {
    const result = await pool.query(
      "UPDATE participants SET full_name=$1, phone_number=$2, status=$3, room_no=$4, dining_seat_no=$5, pagoda_cell_no=$6 WHERE participant_id=$7 RETURNING *",
      [full_name, phone_number, status, room_no, dining_seat_no, pagoda_cell_no, id]
    );
    res.json(result.rows[0]);
  } catch (err) { res.status(500).json({ error: err.message }); }
});

// DELETE PARTICIPANT (NEW!)
...
app.post('/check-in', async (req, res) => {
  // Added pagodaCell and laptop here
  const { courseId, participantId, roomNo, seatNo, laundryToken, mobileLocker, valuablesLocker, language, pagodaCell, laptop } = req.body;
  ...
  try {
    const query = `
      ...
      SET status = 'Arrived', room_no = $1, dining_seat_no = $2, laundry_token_no = $3, 
          ...
          pagoda_cell_no = $7, laptop_details = $8
      WHERE participant_id = $9 AND course_id = $10
      RETURNING *;
    `;
    const values = [roomNo, seatNo, laundryToken, mobileLocker, valuablesLocker, language || 'English', pagodaCell, laptop, participantId, courseId];
    
    const result = await pool.query(query, values);
    if (result.rows.length === 0) return res.status(404).json({ error: "Student not found" });
    res.json(result.rows[0]);
  } catch (err) {
    if (err.code === '23505') res.status(409).json({ error: "Duplicate assignment! Room/Seat/Token already taken." });
