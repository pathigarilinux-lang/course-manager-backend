...
app.post('/check-in', async (req, res) => { const { courseId, participantId, roomNo, seatNo, diningSeatType, laundryToken, mobileLocker, valuablesLocker, language, pagodaCell, laptop, confNo, dhammaSeat, specialSeating } = req.body; try { const clean = (val) => (val && typeof val === 'string' && ['na', 'n/a', 'no', 'none', '-'].includes(val.trim().toLowerCase())) ? null : (val || null); if (roomNo) { const roomCheck = await pool.query( "SELECT p.full_name, c.course_name FROM participants p JOIN courses c ON p.course_id = c.course_id WHERE p.room_no = $1 AND p.status = 'Arrived' AND p.participant_id != $2", [roomNo, participantId] ); if (roomCheck.rows.length > 0) { return res.status(409).json({ error: `🛑 Room ${roomNo} occupied by ${roomCheck.rows[0].full_name} (${roomCheck.rows[0].course_name})` }); } } const studentRes = await pool.query("SELECT gender FROM participants WHERE participant_id = $1", [participantId]); if (studentRes.rows.length > 0 && seatNo) { const gender = studentRes.rows[0].gender; const seatCheck = await pool.query(` SELECT p.full_name FROM participants p WHERE p.course_id = $1 AND p.dining_seat_no = $2 AND p.participant_id != $3 AND LOWER(p.gender) = LOWER($4) `, [courseId, seatNo, participantId, gender]); if (seatCheck.rows.length > 0) { return res.status(409).json({ error: `⚠️ Seat ${seatNo} already taken by ${seatCheck.rows[0].full_name} (${gender})` }); } } const query = ` UPDATE participants SET status = 'Arrived', room_no = $1, dining_seat_no = $2, laundry_token_no = $3, mobile_locker_no = $4, valuables_locker_no = $5, discourse_language = $6, pagoda_cell_no = $7, laptop_details = $8, conf_no = $9, dhamma_hall_seat_no = $10, special_seating = $11, dining_seat_type = $12 WHERE participant_id = $13 AND course_id = $14 RETURNING *; `; const values = [ clean(roomNo), clean(seatNo), clean(laundryToken), clean(mobileLocker), clean(valuablesLocker), language||'English', clean(pagodaCell), laptop, clean(confNo), clean(dhammaSeat), clean(specialSeating), diningSeatType, participantId, courseId ]; const result = await pool.query(query, values); if (result.rows.length === 0) return res.status(404).json({ error: "Student not found" }); res.json(result.rows[0]); } catch (err) { if (err.code === '23505') return res.status(409).json({ error: "Duplicate data detected." }); res.status(500).json({ error: err.message }); } });
app.put('/participants/:id', async (req, res) => { const { id } = req.params; const { full_name, phone_number, status, room_no, dining_seat_no, dining_seat_type, pagoda_cell_no, conf_no, dhamma_hall_seat_no, special_seating, discourse_language, evening_food, medical_info, teacher_notes } = req.body; try { const clean = (val) => (val && ['na','n/a','none'].includes(val.toLowerCase())) ? null : (val || null); const result = await pool.query( `UPDATE participants SET full_name=$1, phone_number=$2, status=$3, room_no=$4, dining_seat_no=$5, pagoda_cell_no=$6, conf_no=$7, dhamma_hall_seat_no=$8, special_seating=$9, discourse_language=$10, dining_seat_type=$11, evening_food=$12, medical_info=$13, teacher_notes=$14 WHERE participant_id=$15 RETURNING *`, [full_name, phone_number, status, clean(room_no), clean(dining_seat_no), clean(pagoda_cell_no), clean(conf_no), clean(dhamma_hall_seat_no), clean(special_seating), discourse_language, dining_seat_type, evening_food, medical_info, teacher_notes, id] ); res.json(result.rows[0]); } catch (err) { if (err.code === '23505') return res.status(409).json({ error: "Duplicate data found." }); res.status(500).json({ error: err.message }); } });
