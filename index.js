...
app.get('/courses/:id/financial-report', async (req, res) => { try { const query = `SELECT p.full_name, p.room_no, p.dining_seat_no, COALESCE(SUM(CASE WHEN e.expense_type ILIKE '%Laundry%' AND e.amount > 0 THEN e.amount ELSE 0 END), 0) as laundry_total, COALESCE(SUM(CASE WHEN e.expense_type NOT ILIKE '%Laundry%' AND e.expense_type NOT ILIKE '%Payment%' AND e.amount > 0 THEN e.amount ELSE 0 END), 0) as shop_total, COALESCE(SUM(CASE WHEN e.amount > 0 THEN e.amount ELSE 0 END), 0) as total_bill, COALESCE(SUM(e.amount), 0) as total_due FROM participants p LEFT JOIN expenses e ON p.participant_id = e.participant_id WHERE p.course_id = $1 GROUP BY p.participant_id, p.full_name, p.room_no, p.dining_seat_no ORDER BY p.full_name ASC`; const result = await pool.query(query, [req.params.id]); res.json(result.rows); } catch (err) { res.status(500).json({ error: err.message }); } });
